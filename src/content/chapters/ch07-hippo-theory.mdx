...
The HiPPO-LegS matrix is lower-triangular and its eigenvalues are the well-separated integers $-1, \dots, -N$ (§7.7), which sounds maximally benign. It is not, and the reason is **non-normality**: $\statemat$ is far from symmetric, so its eigenvectors are highly oblique and the matrix that diagonalizes it is ill-conditioned. The condition number of that eigenvector basis **grows exponentially in $N$** <Cite key="yu2023robustifying" />, so a naive diagonalization of HiPPO-LegS is numerically untrustworthy at the $N$ used in practice — and the sub-quadratic conditioning one *wants* is achieved only by deliberately perturbing the transform (the "robustifying" construction below).

<MarginNote>An earlier draft of this book (and much informal lore) claimed the HiPPO basis conditioning stays *bounded* as $N$ grows. It does **not** — the diagonalizing transform is exponentially ill-conditioned <Cite key="yu2023robustifying" />. Separately, the well-posedness of the singular LegS ODE and the convergence of its discretization schemes are established rigorously in <Cite key="park2024numerical" />; together these are why off-the-shelf integrators lose accuracy at large $N$ and corrected discretizations matter.</MarginNote>
