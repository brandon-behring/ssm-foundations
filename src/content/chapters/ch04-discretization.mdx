...
<MarginNote>Lax-Richtmyer was originally stated for finite-difference schemes for PDEs. The ODE analogue is **Dahlquist's equivalence theorem** for multistep methods — convergence ⇔ consistency + zero-stability <Cite key="dahlquist1956convergence" />; the one-step specialization used here is folklore. See Hairer–Nørsett–Wanner <Cite key="hairer1993ordinary" /> §II.3 for a textbook treatment.</MarginNote>
...
$$
and a single `expm` call extracts both $\discA$ (top-left block) and $\discB$ (top-right block) without ever inverting $\statemat$. The block identity goes back to Van Loan <Cite key="vanloan1978computing" />; the JAX companion `discretization_comparison.py` uses it.
...
Forward-looking SSM connections: ZOH is this book's default in Chapters 7–9, and natively the choice of S5 and Mamba-1/2; the original S4 and S4D papers used bilinear, with the S4D ablations finding the two interchangeable. Exp-trapezoidal is the Mamba-3 scheme, treated in Chapter 10.
