...
<Figure src="/figures/ch06/phase_portrait.png" alt="Phase portrait of the pendulum: closed orbits preserved by symplectic Euler, spirals (energy drift) under RK4." caption="Pendulum $\\hamilton = p^2/2 - \\cos q$ phase portrait over 50 periods at $\\Delta = 0.1$ (large step size, chosen to expose qualitative differences). Symplectic Euler (navy): trajectory stays on a closed orbit (constant modified energy). RK4 (gold): orbit slowly spirals outward (linear energy drift). Both methods are formally stable; the qualitative difference is geometric. Produced by companions/ch06/jax/symplectic_demo.py." />
...
The companion's main output shows two energy-vs-time traces over 100 periods. RK4 drifts approximately linearly; the drift rate is roughly $10^{-5}$ per period at $\stepsize = 0.05$ — consistent with $O(\stepsize^5)$ local error compounded over $\sim 125$ steps per period. Störmer–Verlet's energy stays within a band of width $\sim 10^{-3} = O(\stepsize^2)$, with no secular drift. The contrast — linear drift vs bounded oscillation — is the key takeaway of §6.5.
</details>
