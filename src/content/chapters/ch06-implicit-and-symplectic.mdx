...
- `stiff_demo.py` — van der Pol oscillator at $\mu = 30$ (mildly stiff); compares explicit RK4 (blows up at large $\stepsize$) against backward Euler (stable at every $\stepsize$); emits `stiff_blowup.png`.
