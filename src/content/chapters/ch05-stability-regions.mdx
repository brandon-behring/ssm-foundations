...
The proof is non-trivial and we will not reproduce it; the canonical reference is Hairer–Nørsett–Wanner Chapter II.2 <Cite key="hairer1993ordinary" />. For our purposes the takeaway is that **the order of an RK method is a property of its tableau entries**, derivable by algebraic manipulation alone. You can verify (Exercise 5.1) that RK4's tableau satisfies all 4 order-4 conditions; you can also verify (Exercise 5.2) that an arbitrary two-stage explicit tableau cannot achieve order higher than 2, regardless of how you choose the free parameters.
...
Verify that classical RK4 satisfies the four order-conditions of order 4 — at least the simpler ones: $\sum b_i = 1$, $\sum b_i c_i = 1/2$, $\sum b_i c_i^2 = 1/3$. Use the tableau in §5.1.
