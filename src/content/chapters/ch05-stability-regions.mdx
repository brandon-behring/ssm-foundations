...
<Figure id="ch05:fig:order-verification" src="/figures/ch05/order_verification.png" alt="Log-log error vs step size for forward Euler (slope 1), midpoint RK2 (slope 2), and classical RK4 (slope 4) on a forced damped oscillator." caption="Empirical verification of the Butcher order conditions (§5.2). Each method's local truncation error decreases as $\\Delta^{p+1}$, giving global error $\\Delta^p$ — a $p$-th order method has slope $p$ on the log-log plot. RK1, RK2, and RK4 all match their theoretical orders to within ~5% on this forced linear oscillator. Produced by companions/ch05/jax/order_verification.py." />
